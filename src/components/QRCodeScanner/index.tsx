  ...
  React.useEffect(() => {
    ...
    const initializeCamera = async () => {
      try {
        ...
        // Update loading state after initialization
        setLoading(false);
      } catch (error) {
        // Handle camera access error
        setLoading(false);
        toast.error(
          "Erro ao aceder à câmera. Certifique-se de que permitiu o acesso à câmera e recarregue a página."
        );
      ...
      {loading && (
        <div className="absolute left-0 top-0 flex h-full w-full items-center justify-center bg-black bg-opacity-50">
          <div className="flex flex-col items-center">
            <div className="h-12 w-12 animate-spin rounded-full border-b-2 border-r-2 border-t-2 border-blue-500 border-blue-500"></div>
